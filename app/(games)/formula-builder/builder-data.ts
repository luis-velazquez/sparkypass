...
export const FORMULA_PACKS: FormulaPack[] = [
  {
    id: "free",
    name: "Starter Pack",
    scenarios: [
...
export const SCENARIOS: FormulaScenario[] = FORMULA_PACKS.flatMap((p) => p.scenarios);

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Get all unique pack IDs in order */
export function getPackIds(): string[] {
  return FORMULA_PACKS.map((p) => p.id);
}

/** Get scenarios from unlocked packs only */
export function getUnlockedScenarios(unlockedPacks: string[]): FormulaScenario[] {
  const set = new Set(unlockedPacks);
  return FORMULA_PACKS.filter((p) => set.has(p.id)).flatMap((p) => p.scenarios);
}

/** Fisher-Yates shuffle — returns a new array. */
export function shuffleSteps(steps: FormulaStep[]): FormulaStep[] {
...
/** Map a correct-answer streak to an energize level (0-4). */
